...
class Scene7_VentilationSystemsComparison(Scene):
    def construct(self):
        ...
        def create_cube_graphic(center_pos):
            ...
            cube_label = Text("1 m³", font_size=13, color=WHITE).move_to(center_pos + DOWN * 0.45)
            return VGroup(face_top, face_left, face_right, cube_label)

        # ----------------------------------------------------
        # PANEL 1: FENSTERLÜFTUNG (FREIE LÜFTUNG, OHNE WRG)
        # ----------------------------------------------------
        card1 = RoundedRectangle(
            width=col_w, height=col_h, corner_radius=0.15, color="#FF6B6B", fill_color="#1a1215", fill_opacity=0.85
        ).move_to(pos1)
        ...
        lbl_in1 = Text("Kaltluft (-5°C)", font_size=9, color=BLUE_B).next_to(arr_in1, LEFT, buff=0.02).shift(UP * 0.1)
        ...
        lbl_out1 = Paragraph(
            "Warmluft (+21°C)", "entweicht!", font_size=9, color=ORANGE, alignment="center"
        ).next_to(arr_out1, UP, buff=0.1)

        stat1_wrg = Text("WRG: 0%", font_size=11, color=RED)
        stat1_loss = Text("Lüftungsverlust: 100% (Sehr Hoch)", font_size=10, color=RED_B)
        ...
        card2 = RoundedRectangle(
            width=col_w, height=col_h, corner_radius=0.15, color="#4D96FF", fill_color="#101825", fill_opacity=0.85
        ).move_to(pos2)
        ...
        mvhr_box = RoundedRectangle(
            width=0.8, height=0.45, corner_radius=0.05, color=WHITE, fill_color="#2C3E50", fill_opacity=0.95, stroke_width=2
        ...
        lbl_supply = Text("Vorgewärmte Zuluft (+18°C)", font_size=9, color=GREEN).next_to(arr_supply, RIGHT, buff=0.04)
        ...
        col2 = VGroup(card2, hdr2, cube2, mvhr_box, lbl_mvhr, arr_fresh, arr_exhaust, arr_supply, lbl_supply, box2_stat)

        # ----------------------------------------------------
        # PANEL 3: DEZENTRALE LÜFTUNGSANLAGE (DEZENTRAL)
        # ----------------------------------------------------
        card3 = RoundedRectangle(
            width=col_w, height=col_h, corner_radius=0.15, color="#6BCB77", fill_color="#102018", fill_opacity=0.85
        ).move_to(pos3)
        ...
        lbl_unit = Text("Keramik", font_size=7, color=WHITE).move_to((c_top_l + c_bot_r) / 2)
        unit1 = VGroup(unit_face, lbl_unit)
        ...
        col3 = VGroup(card3, hdr3, cube3, window3, unit1, arr_cyc1, lbl_cyc1, arr_cyc2, lbl_cyc2, box3_stat)
        ...
        table_box = RoundedRectangle(
            width=13.2, height=0.7, corner_radius=0.1, color=YELLOW, fill_color="#2b2510", fill_opacity=0.95
