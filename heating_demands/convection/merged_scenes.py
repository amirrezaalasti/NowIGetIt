...
class Scene1(Scene):
    def construct(self):
        ...
        label_inside = VGroup(txt_innen, txt_warm).arrange(DOWN, buff=0.15).move_to([-1.2, -0.5, 0])

        txt_aussen = Text("Außen", font_size=20, color=BLUE_B)
        txt_kalt = Text("Kalt", font_size=16, color=BLUE_B)
        label_outside = VGroup(txt_aussen, txt_kalt).arrange(DOWN, buff=0.15).move_to([2.6, -0.5, 0])
...
class InteriorBuildingVolume(Scene):
    def construct(self):
        ...
        self.add(ground, house_outline, window, blue_dots, orange_dots)
        self.play(Write(title), run_time=1.0)
        self.wait(0.5)
...
class Scene3(Scene):
    def construct(self):
        ...
        house_fill = Polygon(*house_points, fill_color=COLOR_V, fill_opacity=0.3, stroke_width=0)
        ...
        n_subtext = VGroup(line1, line2).arrange(DOWN, buff=0.08).next_to(clock_icon, DOWN, buff=0.4)
        ...
        rate_caption = Text("1 Luftwechsel = Vollständiges Volumen ausgetauscht", font_size=20, color=COLOR_COLD).to_edge(DOWN, buff=0.6)

        self.play(
            Rotate(hand, angle=-2 * PI, about_point=clock_center, rate_func=linear),
            house_fill.animate.set_fill(COLOR_COLD, opacity=0.3),
...
class Scene4(Scene):
    def construct(self):
        ...
        coil_label = Text("Heizelement (DIN 4108-6)", font_size=15, color=RED_B).next_to(coil, DOWN, buff=0.15)
...
class Scene5(Scene):
    def construct(self):
        ...
        face_top = Polygon(
            c_center, tl_pt, top_pt, tr_pt,
            fill_color=ORANGE, fill_opacity=0.35, stroke_color=ORANGE, stroke_width=2
        )
        face_left = Polygon(
            c_center, tl_pt, bl_pt, bot_pt,
            fill_color="#E65100", fill_opacity=0.45, stroke_color=ORANGE, stroke_width=2
        )
        face_right = Polygon(
            c_center, tr_pt, br_pt, bot_pt,
            fill_color="#BF360C", fill_opacity=0.55, stroke_color=ORANGE, stroke_width=2
        )

        cube = VGroup(face_top, face_left, face_right)
        cube_label = Text("1 m³", font_size=22, color=WHITE).move_to(face_top.get_center())
        ...
        t_inside = Text("T_innen  (Innentemperatur)", color="#FF6B6B", font_size=18).shift(UP * 0.3 + RIGHT * 1.2)
        t_outside = Text("T_außen (Außentemperatur)", color="#4D96FF", font_size=18).shift(DOWN * 1.5 + RIGHT * 1.2)
...
class Scene6_HeatRecoveryIntro(Scene):
    def construct(self):
        ...
        formula_old = VGroup(
            eq_phi1, eq_v1, eq_dot11, eq_n1, eq_dot12, eq_c1, eq_dot13, eq_dt1
        ).arrange(RIGHT, buff=0.08).next_to(title, DOWN, buff=0.4)

        old_label = Text("Formel ohne Wärmerückgewinnung (η = 0%)", font_size=14, color=GREY_A).next_to(formula_old, DOWN, buff=0.15)
        ...
        formula_new = VGroup(
            eq_phi2, eq_v2, eq_dot21, eq_n2, eq_dot22, eq_eta2, eq_dot23, eq_c2, eq_dot24, eq_dt2
        ).arrange(RIGHT, buff=0.08).next_to(title, DOWN, buff=0.4)

        box_new = SurroundingRectangle(formula_new, color="#FF9F43", buff=0.15, corner_radius=0.1)
        new_label = Text("Reduzierte Heizlast durch Wärmerückgewinnungsgrad η_WRG", font_size=14, color=GREEN).next_to(box_new, DOWN, buff=0.15)
        ...
        bar1 = RoundedRectangle(width=4.0, height=0.35, corner_radius=0.05, color=RED, fill_color=RED, fill_opacity=0.6)
        bar_text1 = Text("100% Lüftungsverlust", font_size=12, color=WHITE).move_to(bar1)
        bar_group1 = VGroup(bar_label1, VGroup(bar1, bar_text1)).arrange(RIGHT, buff=0.3)

        # Bar 2: With WRG (η = 80% -> 20% loss remaining)
        bar_label2 = Text("Mit WRG (η = 80%)", font_size=14, color=GREEN)
        bar2_saved = RoundedRectangle(width=3.2, height=0.35, corner_radius=0.05, color=GREEN, fill_color=GREEN, fill_opacity=0.6)
        bar_text2_saved = Text("80% Eingespart", font_size=12, color=WHITE).move_to(bar2_saved)
        bar2_loss = RoundedRectangle(width=0.8, height=0.35, corner_radius=0.05, color=RED, fill_color=RED, fill_opacity=0.6).next_to(bar2_saved, RIGHT, buff=0.0)
        bar_text2_loss = Text("20%", font_size=11, color=WHITE).move_to(bar2_loss)
        bar_group2 = VGroup(bar_label2, VGroup(bar2_saved, bar_text2_saved, bar2_loss, bar_text2_loss)).arrange(RIGHT, buff=0.35)

        comparison_visual = VGroup(bar_group1, bar_group2).arrange(DOWN, aligned_edge=LEFT, buff=0.3).shift(DOWN * 0.7)
...
class Scene7_VentilationSystemsComparison(Scene):
    def construct(self):
        ...
        def create_base_house(center_pos):
            wall_top = Rectangle(
                width=2.4, height=0.35, color=GREY_A, fill_color="#333A42", fill_opacity=0.9, stroke_width=2
            ).move_to(center_pos + UP * 0.85)
            wall_bot = Rectangle(
                width=2.4, height=0.35, color=GREY_A, fill_color="#333A42", fill_opacity=0.9, stroke_width=2
            ).move_to(center_pos + DOWN * 0.85)

            lbl_outside = Text("Außen (Kalt)", font_size=10, color=BLUE_B).move_to(center_pos + LEFT * 0.7 + UP * 1.15)
            lbl_inside = Text("Innen (Warm)", font_size=10, color=ORANGE).move_to(center_pos + RIGHT * 0.7 + UP * 1.15)
        ...
        rect1 = RoundedRectangle(
            ...
            color="#FF6B6B", fill_color="#1a1215", fill_opacity=0.85
        )
        title1 = Text("1. Fensterlüftung", font_size=17, color="#FF6B6B")
        eta1 = Text("η_WRG = 0%  (Freie Lüftung)", font_size=12, color=RED_B)
        hdr1 = VGroup(title1, eta1).arrange(DOWN, buff=0.08).move_to(rect1.get_top() + DOWN * 0.45)
        ...
        arr_warm_out = Arrow(center1 + RIGHT * 0.95 + UP * 0.3, center1 + LEFT * 0.95 + UP * 0.3, color=ORANGE, stroke_width=3, buff=0)
        arr_cold_in = Arrow(center1 + LEFT * 0.95 + DOWN * 0.3, center1 + RIGHT * 0.95 + DOWN * 0.3, color=BLUE_B, stroke_width=3, buff=0)
        lbl_out1 = Text("Warm Raus", font_size=10, color=ORANGE).next_to(arr_warm_out, UP, buff=0.04)
        lbl_in1 = Text("Kalt Rein", font_size=10, color=BLUE_B).next_to(arr_cold_in, DOWN, buff=0.04)

        lbl_loss1 = Text("100% Lüftungsverlust", font_size=13, color=RED).move_to(rect1.get_bottom() + UP * 0.35)

        col1 = VGroup(rect1, hdr1, base1, arr_warm_out, arr_cold_in, lbl_out1, lbl_in1, lbl_loss1).move_to(LEFT * 4.1 + card_y)
        ...
        rect2 = RoundedRectangle(
            width=card_w, height=card_h, corner_radius=0.15,
            color="#4D96FF", fill_color="#101825", fill_opacity=0.85
        )
        title2 = Text("2. Zentrale WRG", font_size=17, color="#4D96FF")
        eta2 = Text("η_WRG = 80 - 90% (Zentral)", font_size=12, color=GREEN_B)
        hdr2 = VGroup(title2, eta2).arrange(DOWN, buff=0.08).move_to(rect2.get_top() + DOWN * 0.45)

        center2 = rect2.get_center() + DOWN * 0.25
        base2 = create_base_house(center2)

        # Central Heat Exchanger Plate in channel
        hx = RoundedRectangle(width=0.9, height=0.9, corner_radius=0.08, color=WHITE, fill_color="#1A2638", fill_opacity=0.95, stroke_width=2).move_to(center2)
        ...
        arr_ex_in = Arrow(center2 + RIGHT * 0.95 + UP * 0.25, center2 + RIGHT * 0.45 + UP * 0.25, color=ORANGE, stroke_width=3, buff=0)
        arr_ex_out = Arrow(center2 + LEFT * 0.45 + UP * 0.25, center2 + LEFT * 0.95 + UP * 0.25, color=BLUE_B, stroke_width=3, buff=0)
        
        arr_sup_in = Arrow(center2 + LEFT * 0.95 + DOWN * 0.25, center2 + LEFT * 0.45 + DOWN * 0.25, color=BLUE_B, stroke_width=3, buff=0)
        arr_sup_out = Arrow(center2 + RIGHT * 0.45 + DOWN * 0.25, center2 + RIGHT * 0.95 + DOWN * 0.25, color=ORANGE, stroke_width=3, buff=0)

        lbl_loss2 = Text("Bis zu 90% Ersparnis!", font_size=13, color=GREEN).move_to(rect2.get_bottom() + UP * 0.35)

        col2 = VGroup(rect2, hdr2, base2, hx, lbl_hx, arr_ex_in, arr_ex_out, arr_sup_in, arr_sup_out, lbl_loss2).move_to(card_y)
        ...
        rect3 = RoundedRectangle(
            width=card_w, height=card_h, corner_radius=0.15,
            color="#6BCB77", fill_color="#102018", fill_opacity=0.85
        )
        title3 = Text("3. Dezentrale WRG", font_size=17, color="#6BCB77")
        eta3 = Text("η_WRG = 70 - 85% (Pendel)", font_size=12, color=GREEN_B)
        hdr3 = VGroup(title3, eta3).arrange(DOWN, buff=0.08).move_to(rect3.get_top() + DOWN * 0.45)

        center3 = rect3.get_center() + DOWN * 0.25
        base3 = create_base_house(center3)

        # Ceramic Heat Accumulator Core in channel
        ceramic = RoundedRectangle(width=0.9, height=0.9, corner_radius=0.08, color=YELLOW, fill_color=ORANGE, fill_opacity=0.65, stroke_width=2).move_to(center3)
        lbl_ceramic = Text("Keramik-\nSpeicher", font_size=9, color=WHITE).move_to(ceramic)

        # Reversing Push-Pull flow
        push_pull_arr = DoubleArrow(center3 + LEFT * 0.95, center3 + RIGHT * 0.95, color=YELLOW, stroke_width=3, buff=0).move_to(center3 + DOWN * 0.52)
        lbl_cycle = Text("70s Raus / 70s Rein", font_size=10, color=YELLOW).next_to(push_pull_arr, DOWN, buff=0.04)

        lbl_loss3 = Text("Regenerativer Pendelbetrieb", font_size=12, color=GREEN).move_to(rect3.get_bottom() + UP * 0.35)

        col3 = VGroup(rect3, hdr3, base3, ceramic, lbl_ceramic, push_pull_arr, lbl_cycle, lbl_loss3).move_to(RIGHT * 4.1 + card_y)
        ...
        summary_box = RoundedRectangle(
            width=12.2, height=0.6, corner_radius=0.1, color=YELLOW, fill_color="#2b2510", fill_opacity=0.9
