...
# Add project root to path to import backend utilities
...
def main():
    ...
    scenes = [
        ...
        ("InteriorBuildingVolume", base_dir / "scene_2_audio.mp3"),
        ("Scene3", base_dir / "scene_3_audio.mp3"),
        ("Scene4", base_dir / "scene_4_audio.mp3"),
        ("Scene5", base_dir / "scene_5_audio.mp3"),
    ]
    
    muxed_clips = []
    
    for idx, (scene_name, audio_path) in enumerate(scenes, start=1):
        print(f"--- Rendering {scene_name} (Scene {idx}/5) ---")
        
        # Render scene in high quality (1080p60)
        render_cmd = [
        ...
        # Mux audio with freeze-frame padding if needed
    ...
    final_output = base_dir / "Full_Convection_Video_HQ.mp4"
