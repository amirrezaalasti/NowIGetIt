...
class Scene1(Scene):
    def construct(self):
        ...
        windows = VGroup(
            ...
            Rectangle(width=1.2, height=0.8).move_to([1.8, -1.45 + y_off, 0]),
        ).set_color(GREY_LINE).set_stroke(width=1.5, opacity=0.7)

        house = VGroup(outer_walls, interior, windows)

        # Crescent Moon (Upper-left corner)
        moon_center = np.array([-5.2, 2.8, 0])
        moon_base = Circle(radius=0.42, color="#FDE047", fill_opacity=0.85).move_to(moon_center).set_stroke(width=0)
        moon_shadow = Circle(radius=0.38, color="#0f1115", fill_opacity=1.0).move_to(moon_center + np.array([0.16, 0.12, 0])).set_stroke(width=0)
        ...
        # Subtitle Text
        subtitle = Text(
            "Exterior Cold Ambient Air vs. Internal Heat Sources",
            font_size=22,
            color=GREY_A
        ).to_edge(DOWN, buff=0.5)

        # Animation Sequence
        # Beat 1: Fade in house cross-section structure and crescent moon
        self.play(
        ...
        # Beat 2: Introduce cold exterior particles
        self.play(
            FadeIn(particles, run_time=1.0)
        )

        # Beat 3: Display subtitle with smooth fade-and-shift
        self.play(
            FadeIn(subtitle, shift=UP * 0.2, run_time=1.2)
        )

        # Animate subtle, slow drift of exterior cold air particles
        self.play(
...
class Scene2(Scene):
    def construct(self):
        ...
        # 2. Minimalist House Cross-Section (GREY_B line geometry)
        ...
        # 3. Seated Human Silhouette & Chair
        head = Circle(radius=0.22, color="#F472B6", fill_color="#F472B6", fill_opacity=0.3, stroke_width=2)
        ...
        chair_back = Line([-0.15, -0.8, 0], [-0.15, -1.55, 0], color=GREY_C, stroke_width=2)
        chair_seat = Line([-0.15, -1.55, 0], [0.4, -1.55, 0], color=GREY_C, stroke_width=2)
        chair_leg = Line([0.1, -1.55, 0], [0.1, -2.2, 0], color=GREY_C, stroke_width=2)
        chair = VGroup(chair_back, chair_seat, chair_leg)

        # 4. Warm Pink Glow Effect
        glow_1 = Circle(radius=0.5, color="#F472B6", stroke_width=0, fill_opacity=0.18).move_to([0.1, -1.2, 0])
        glow_2 = Circle(radius=0.9, color="#F472B6", stroke_width=0, fill_opacity=0.09).move_to([0.1, -1.2, 0])
        glow_3 = Circle(radius=1.4, color="#F472B6", stroke_width=0, fill_opacity=0.04).move_to([0.1, -1.2, 0])
        glow = VGroup(glow_3, glow_2, glow_1)

        # 5. Upward Wavy Heat Curves
        ...
        # 6. Variable Labels and Text
        phi_p = Text("Φ_p", color="#F472B6", font_size=38)
        phi_p.move_to([1.3, -0.5, 0])
        
        desc_text = Text("Occupants: ~100 W", color=WHITE, font_size=20)
        desc_text.next_to(phi_p, DOWN, aligned_edge=LEFT, buff=0.15)

        eq_zone_title = Text("Internal Heat Gains", color=GREY_B, font_size=22)
        eq_zone_title.to_corner(UL).shift(RIGHT * 0.5 + DOWN * 0.3)

        # --- ANIMATION BEATS ---

        # Beat 1: Draw house geometry and human silhouette
        self.play(
        ...
        # Beat 2: Animate warm pink glow and rising heat waves
        self.play(
            FadeIn(glow, run_time=1.2),
            Create(heat_waves, run_time=1.5)
        )
        self.play(
        ...
        # Beat 3: Display bold pink label Phi_p next to occupant
        self.play(
            Write(phi_p),
        ...
        # Beat 4: Float Phi_p up to top-left equation zone
        self.play(
            Write(eq_zone_title),
            run_time=0.8
        )
        
        phi_p_target = Text("Φ_p  (Occupants = ~100W)", color="#F472B6", font_size=24)
        phi_p_target.next_to(eq_zone_title, DOWN, aligned_edge=LEFT, buff=0.3)

        self.play(
            Transform(phi_p, phi_p_target),
            FadeOut(desc_text),
            run_time=1.7
        )
...
class Scene3(Scene):
    def construct(self):
        ...
        # Header / Equation Bar Elements
        eq_title = Text("Internal Heat Gain", font_size=22, color=GREY_B)
        ...
        # Architectural Cross-Section Geometry
        floor = Line(LEFT * 5 + DOWN * 2, RIGHT * 4.5 + DOWN * 2, color=GREY_B, stroke_width=2)
        wall = Line(RIGHT * 4 + DOWN * 2, RIGHT * 4 + UP * 2.5, color=GREY_B, stroke_width=2)
        ceiling = Line(LEFT * 5 + UP * 2.5, RIGHT * 4 + UP * 2.5, color=GREY_B, stroke_width=2)
        room = VGroup(floor, wall, ceiling)
        self.add(room)

        # Occupant Silhouette
        head = Circle(radius=0.22, color=pink_color, fill_opacity=0.2).move_to(LEFT * 2.5 + DOWN * 0.5)
        body = Line(LEFT * 2.5 + DOWN * 0.72, LEFT * 2.5 + DOWN * 1.4, color=pink_color, stroke_width=3)
        legs = VGroup(
            Line(LEFT * 2.5 + DOWN * 1.4, LEFT * 2.0 + DOWN * 1.4, color=pink_color, stroke_width=3),
            Line(LEFT * 2.0 + DOWN * 1.4, LEFT * 2.0 + DOWN * 2.0, color=pink_color, stroke_width=3)
        )
        ...
        human_wave1 = ParametricFunction(
            ...
            t_range=[0, 1.5], color=pink_color, stroke_width=1.5
        ).set_opacity(0.5)
        ...
        desk_surface = Line(LEFT * 0.8 + DOWN * 1.1, RIGHT * 1.8 + DOWN * 1.1, color=GREY_B, stroke_width=2)
        desk_leg1 = Line(LEFT * 0.6 + DOWN * 1.1, LEFT * 0.6 + DOWN * 2.0, color=GREY_B, stroke_width=2)
        desk_leg2 = Line(RIGHT * 1.6 + DOWN * 1.1, RIGHT * 1.6 + DOWN * 2.0, color=GREY_B, stroke_width=2)
        desk = VGroup(desk_surface, desk_leg1, desk_leg2)

        # Laptop Geometry
        laptop_base = Line(LEFT * 0.4 + DOWN * 1.1, RIGHT * 0.2 + DOWN * 1.1, color=GREY_B, stroke_width=2.5)
        laptop_screen = Line(RIGHT * 0.2 + DOWN * 1.1, RIGHT * 0.3 + DOWN * 0.6, color=GREY_B, stroke_width=2.5)
        laptop = VGroup(laptop_base, laptop_screen)

        # Server Rack Geometry
        server_box = Rectangle(width=0.8, height=1.3, color=GREY_B, stroke_width=2).move_to(RIGHT * 1.0 + DOWN * 0.45)
        server_lines = VGroup(*[
            Line(RIGHT * 0.68 + DOWN * (0.1 + i * 0.2), RIGHT * 1.32 + DOWN * (0.1 + i * 0.2), color=GREY_B, stroke_width=1)
            for i in range(4)
        ])
        server_leds = VGroup(*[
            Dot(RIGHT * 0.76 + DOWN * (0.1 + i * 0.2), radius=0.03, color=blue_color)
            for i in range(4)
        ])
        server = VGroup(server_box, server_lines, server_leds)

        # Wall Socket
        socket = Square(side_length=0.2, color=GREY_B, fill_opacity=0.3).move_to(RIGHT * 4.0 + DOWN * 1.1)
        socket_slots = VGroup(
            Line(RIGHT * 3.96 + DOWN * 1.05, RIGHT * 3.96 + DOWN * 1.15, color=GREY_B, stroke_width=1),
            Line(RIGHT * 4.04 + DOWN * 1.05, RIGHT * 4.04 + DOWN * 1.15, color=GREY_B, stroke_width=1)
        )

        # Render Equipment & Furniture
        self.play(
            Create(desk),
            Create(laptop),
            Create(server),
        ...
        spark = Star(n=8, outer_radius=0.25, inner_radius=0.08, color=blue_color, fill_opacity=0.9)
        spark.move_to(socket.get_center())

        self.play(FadeIn(spark, scale=0.3), run_time=0.3)
        self.play(
            spark.animate.scale(1.8).set_opacity(0),
            run_time=0.4
        )
        self.remove(spark)

        # Equipment Glow & Temperature Rise
        self.play(
        ...
        # Floating Equipment Variable Label Phi_e
        ...
        # Move label to top equation bar next to Phi_p
        self.play(
            Write(plus_sign),
            Transform(phi_e_spawn, phi_e_target),
            run_time=1.5
        )
...
class LightingHeat(Scene):
    def construct(self):
        ...
        # 1. Architectural Cross-Section Elements
        ceiling = Line(LEFT * 6 + UP * 2.5, RIGHT * 6 + UP * 2.5, color=COLOR_STRUCTURE, stroke_width=4)
        floor = Line(LEFT * 6 + DOWN * 2.2, RIGHT * 6 + DOWN * 2.2, color=COLOR_STRUCTURE, stroke_width=4)
        ...
        # Animation Beat 1: Draw cross-section and hanging fixture
        self.play(
            Create(ceiling),
            Create(floor),
            run_time=1.2
        ...
        # Animation Beat 2: Turn on fixture with glowing cone of illumination
        self.play(
            FadeIn(bulb),
        ...
        # Animation Beat 3: Photon stream downward & transformation to heat waves
        photon_start_x = [-0.3, -0.15, 0.0, 0.15, 0.3]
        photon_end_x = [-1.8, -0.9, 0.0, 0.9, 1.8]

        photons = VGroup(*[
            Dot(point=UP * 0.85 + RIGHT * sx, radius=0.07, color=COLOR_LIGHTING)
            for sx in photon_start_x
        ])

        # Generate rising thermal heat wave curves
        heat_waves = VGroup()
        ...
        # Animate photons traveling down to surface
        self.play(
            LaggedStart(*[
                photon.animate.move_to(DOWN * 2.2 + RIGHT * ex)
                for photon, ex in zip(photons, photon_end_x)
            ], lag_ratio=0.12),
            run_time=1.8
        )

        # Photons impact floor and convert into rising thermal energy waves
        self.play(
            FadeOut(photons),
            Create(heat_waves),
            run_time=1.4
        ...
        self.play(
            heat_waves.animate.shift(UP * 0.3).set_opacity(0.6),
            run_time=1.2
        )

        # Animation Beat 4: Formulate equation Phi_p + Phi_e + Phi_l
        phi_l_label = Text("Φ_l", color=COLOR_LIGHTING, font_size=36)
        phi_l_label.next_to(shade, RIGHT, buff=0.6)

        # Assemble formula components at top of screen
        ...
        full_equation = VGroup(eq_p, eq_plus1, eq_e, eq_plus2, eq_l_target).arrange(RIGHT, buff=0.12).to_edge(UP, buff=0.5)
        partial_equation = VGroup(eq_p, eq_plus1, eq_e, eq_plus2)

        # Reveal local label and partial formula
        self.play(
            Write(phi_l_label),
            FadeIn(partial_equation),
            run_time=1.2
        )
        self.wait(0.4)

        # Float Phi_l to top line to complete internal heat gain sum
        self.play(
            Transform(phi_l_label, eq_l_target),
            run_time=1.5
        )

        # Outline complete expression
        bounding_box = SurroundingRectangle(full_equation, color=COLOR_LIGHTING, buff=0.15, corner_radius=0.1)
        bounding_box.set_stroke(width=1.5)

        total_title = Text("Total Internal Heat Gain", color=WHITE, font_size=20).next_to(bounding_box, DOWN, buff=0.18)
...
class Scene5(Scene):
    def construct(self):
        ...
        # Define specific heat source color palette
        COLOR_PEOPLE = "#F472B6"  # Pink
        COLOR_EQUIP = "#38BDF8"   # Blue
        COLOR_LIGHT = "#FBBF24"   # Yellow
        COLOR_FLUX = "#F97316"    # Orange

        # Architectural baseline graphic
        bg_line = Line(LEFT * 6, RIGHT * 6, color=GREY_E, stroke_width=1.5).to_edge(DOWN, buff=1.2)
        self.add(bg_line)

        # Helper function for subscripted variable text mobjects
        ...
        # Section Title
        title = Text("Total Internal Heat Gain", font_size=32, color=WHITE)
        title.to_edge(UP, buff=0.8)
        self.play(Write(title), run_time=1.0)

        # Initial source terms floating at top
        p_var = make_var("Φ", "p", COLOR_PEOPLE)
        e_var = make_var("Φ", "e", COLOR_EQUIP)
        l_var = make_var("Φ", "l", COLOR_LIGHT)

        p_label = Text("Occupants", font_size=16, color=COLOR_PEOPLE).next_to(p_var, UP, buff=0.2)
        e_label = Text("Equipment", font_size=16, color=COLOR_EQUIP).next_to(e_var, UP, buff=0.2)
        l_label = Text("Lighting", font_size=16, color=COLOR_LIGHT).next_to(l_var, UP, buff=0.2)

        p_group = VGroup(p_var, p_label)
        e_group = VGroup(e_var, e_label)
        l_group = VGroup(l_var, l_label)

        sources = VGroup(p_group, e_group, l_group).arrange(RIGHT, buff=1.2).shift(UP * 1.5)
        ...
        # Construct Master Equation: Φ_int = Φ_p + Φ_e + Φ_l
        ...
        master_eq = VGroup(
            phi_int, eq_sign1,
            p_var_eq, plus1, e_var_eq, plus2, l_var_eq
        ).arrange(RIGHT, buff=0.2).move_to(ORIGIN)

        self.play(
            FadeOut(p_label), FadeOut(e_label), FadeOut(l_label),
        ...
        # Prepare Internal Heat Flux Density elements
        density_title = Text("Internal Heat Flux Density", font_size=32, color=WHITE)
        density_title.to_edge(UP, buff=0.8)

        # Elements for fraction: (Φ_p + Φ_e + Φ_l) / A
        ...
        area_var = Text("A", font_size=34, color=WHITE)
        ...
        # Transform to Heat Flux Density formula
        self.play(
        ...
        # Add unit tag below formula
        units = Text("Watts per square meter [W/m²]", font_size=20, color=GREY_B)
        units.next_to(full_density_eq, DOWN, buff=0.7)
        self.play(FadeIn(units, shift=UP * 0.2), run_time=1.0)
        self.wait(1.0)

        # Glowing orange highlight bounding box around q_int
        q_box = SurroundingRectangle(q_int_var, color=COLOR_FLUX, buff=0.12, corner_radius=0.08, stroke_width=2)
        q_box_glow = SurroundingRectangle(q_int_var, color=COLOR_FLUX, buff=0.18, corner_radius=0.12, stroke_width=4)
        ...
        # Cleanly fade out all elements to black
        self.play(
