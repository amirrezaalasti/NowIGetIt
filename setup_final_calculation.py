...
from backend.code_utils import clean_manim_code

source_dir = Path("artifacts/eb5113df6235/scenes")
dest_dir = Path("heating_demands/final_calculation")
dest_dir.mkdir(parents=True, exist_ok=True)

scenes = sorted(source_dir.glob("scene_*"), key=lambda p: [int(c) if c.isdigit() else c for c in re.split(r'(\d+)', p.name)])
...
# Add Master FullFinalCalculationVideo class
merged_code += """
...
print(f"Merged scenes for Final Calculation successfully. Scene classes: {[c[0] for c in class_info]}")

# Create build_full_video.py for Final Calculation
...
build_script += """    ]
    ...
    final_output = base_dir / "Full_Final_Calculation_Video_HQ.mp4"
...
print("build_full_video.py created for Final Calculation.")
