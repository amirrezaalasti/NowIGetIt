...
of picking numbers by eye, per the standardized-template rules shared with
``generate-manim-tutorial``.

NowIGetIt adaptation: Pango kerning (render-large-then-scale) is owned by the
host Text shim in ``backend.code_utils`` — do not re-patch ``Text.__init__`` here.
"""
...
#region Pango kerning
# Host Text shim (``backend.code_utils``) already rasterises ≥48px then scales.
# A second ``Text.__init__`` wrap here would double-scale and fight the shim.


def install_text_kerning_fix() -> None:
    """No-op in NowIGetIt — kerning is applied by the host Text shim."""
    return
