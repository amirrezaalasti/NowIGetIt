"""🎨 Shared Manim helpers for NowIGetIt scene codegen (from tutorial house style).

Palette, pedagogy visuals, the dedicated formula panel, and optional narration
timing helpers — the single source every generated scene imports from instead
of re-declaring its own.

NowIGetIt notes:
- VO timing comes from the beat timeline (TTS is recorded first). Prefer matching
  ``self.play(run_time=...)`` to that timeline; ``hold_for`` / ``NARRATION`` are
  optional and mainly for standalone tutorial folders.
- Subtitles are burned in by compose — skip ``caption_bar`` / ``swap_caption``
  unless you intentionally want an on-screen caption *in addition* to burn-in.
  Layout bands below leave the former caption zone free for content/formulas.
"""
...
#region Layout zones
# Fixed horizontal bands every beat respects, top to bottom: the title block,
# free content, then formula_panel. NowIGetIt burns VO as subtitles in compose,
# so we do not reserve a caption_bar band by default (use SAFE_BOTTOM_CAPTION
# if you opt into caption_bar).
SAFE_TOP = 2.58
SAFE_BOTTOM = -3.20
SAFE_BOTTOM_CAPTION = -2.60  # when using caption_bar
...
# Closer to the bottom edge when caption_bar is unused (NowIGetIt default).
# Use ~1.7 if you also show caption_bar so the two slots never overlap.
FORMULA_PANEL_EDGE_BUFF = 0.55
