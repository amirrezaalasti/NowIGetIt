...
```bash
SKILL=.cursor/skills/scene-audio-sync
...
```bash
cp -R .cursor/skills/scene-audio-sync /path/to/other-repo/.cursor/skills/
python3 -m pip install 'edge-tts>=6.1.0' 'httpx>=0.27.0'
# ensure ffmpeg is installed (brew/apt/winget)
bash /path/to/other-repo/.cursor/skills/scene-audio-sync/scripts/install_check.sh
```
